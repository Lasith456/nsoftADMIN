        ...
        <form action="{{ route('invoices.storeAgent') }}" method="POST">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    ...
                    <select id="agent_id" name="agent_id" x-model="selectedAgentId"
                        class="mt-1 block w-full dark:bg-gray-900 rounded-md py-2 px-3 border-gray-300 dark:border-gray-600"
                        required>
                ...
                <div class="space-y-2 max-h-60 overflow-y-auto border dark:border-gray-700 p-2 rounded-md">
                    <template x-if="availableItems.length > 0">
                        <template x-for="item in availableItems" :key="item.id">
                            <label class="flex items-center p-2 rounded-md hover:bg-gray-100 dark:hover:bg-gray-700">
                                <input type="checkbox" name="delivery_item_ids[]" :value="item.id"
                                       class="dark:bg-gray-900 rounded">
                                <span class="ml-3 text-sm"
                                      x-text="`${item.delivery_note.delivery_note_id}: ${item.quantity_from_agent} x ${(item.product?.name ?? '')}`"></span>
                            </label>
                        </template>
                    </template>
                    ...
                    <template x-if="availableItems.length === 0">
                        <p class="text-sm text-gray-500 dark:text-gray-400 p-2">
                            No uninvoiced items available for this agent.
                        </p>
                    </template>
                </div>
            </div>
...
document.addEventListener('alpine:init', () => {
    Alpine.data('agentInvoiceForm', () => ({
        agents: @json($agents->keyBy('id')), // keyed by id
        selectedAgentId: '',
