...
<div class="bg-white shadow-md rounded-lg p-4">
    ...
    {{-- Filters + Search --}}
    <form x-data x-ref="filterForm" action="{{ route('delivery-notes.index') }}" method="GET" class="flex flex-col md:flex-row justify-between items-start md:items-center mb-4 space-y-2 md:space-y-0 md:space-x-3">
        ...
        <table class="w-full min-w-full divide-y divide-gray-200">
            ...
            <tbody class="bg-white divide-y divide-gray-200">
                ...
                <tr>
                    ...
                    <td class="px-4 py-2 whitespace-nowrap text-sm text-black">{{ $deliveryNote->purchaseOrders->first()?->customer?->customer_name ?? 'N/A' }}</td>
                    ...
                    <td class="px-4 py-2 whitespace-nowrap text-right text-sm font-medium">
                        ...
                        {{-- Update Status (only for processing) --}}

                        {{-- Only show status actions if processing --}}
@if($deliveryNote->status === 'processing')
    {{-- Mark as Delivered --}}
    <form action="{{ route('delivery-notes.updateStatus', $deliveryNote->id) }}" method="POST" class="inline-flex mr-3"
          onsubmit="return confirm('Mark this Delivery Note as Delivered?')">
        @csrf
        <input type="hidden" name="status" value="delivered">
        <button type="submit" class="text-green-600 hover:text-green-800" title="Mark as Delivered">
    ...
    {{-- Cancel --}}
    <form action="{{ route('delivery-notes.updateStatus', $deliveryNote->id) }}" method="POST" class="inline-flex"
          onsubmit="return confirm('Cancel this Delivery Note?')">
        @csrf
