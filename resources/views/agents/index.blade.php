...
<div class="bg-white shadow-md rounded-lg p-4">
    ...
    <div class="flex flex-col md:flex-row justify-between items-start md:items-center mb-4 border-b border-gray-200 pb-3">
        <div>
            <h2 class="text-2xl font-bold text-black">Agents</h2>
    ...
    {{-- Search Form --}}
    <form x-data x-ref="searchForm" action="{{ route('agents.index') }}" method="GET" class="mb-4">
    ...
    <div class="overflow-x-auto">
        <table class="w-full min-w-full divide-y divide-gray-200">
            <thead class="bg-gray-50">
                <tr>
                    <th scope="col" class="px-4 py-2 text-left text-xs font-medium text-black uppercase tracking-wider">Agent ID</th>
                    <th scope="col" class="px-4 py-2 text-left text-xs font-medium text-black uppercase tracking-wider w-2/5">Name</th>
                    <th scope="col" class="px-4 py-2 text-left text-xs font-medium text-black uppercase tracking-wider">Assigned Product</th>
                    <th scope="col" class="px-4 py-2 text-left text-xs font-medium text-black uppercase tracking-wider">Contact No</th>
            ...
            <tbody class="bg-white divide-y divide-gray-200">
               ...
               <tr>
                    ...
                    <td class="px-4 py-2 whitespace-nowrap text-right text-sm font-medium">
                        <form action="{{ route('agents.destroy',$agent->id) }}" method="POST" class="flex justify-end items-center space-x-4">
                            <a href="{{ route('agents.show',$agent->id) }}" class="text-blue-600 hover:text-blue-800" title="Show">
                                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"></path><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"></path></svg>
                            </a>
                            <a href="{{ route('payments.history.agentPayments', $agent->id) }}" 
                            ...
                            @can('agent-edit')
                                <a href="{{ route('agents.edit',$agent->id) }}" class="text-green-600 hover:text-green-800" title="Edit">
                            ...
                            @method('DELETE')
                            @can('agent-delete')
                                <button type="submit" class="text-red-600 hover:text-red-800" title="Delete" onclick="return confirm('Are you sure you want to delete this agent?')">
