    ...
    <div class="bg-white dark:bg-gray-800 shadow-md rounded-lg p-4">
        <div class="flex justify-between items-center mb-4 pb-3 border-b dark:border-gray-700">
            ...
            <div class="flex items-center space-x-2">
                <a href="{{ route('receive-notes.index') }}" class="inline-flex items-center px-4 py-2 bg-gray-200 dark:bg-gray-700 rounded-md font-semibold text-xs text-gray-800 dark:text-gray-200 uppercase">Back</a>
                <button type="submit" form="receive-note-form" class="inline-flex items-center px-4 py-2 bg-blue-600 border rounded-md font-semibold text-xs text-white uppercase hover:bg-blue-700">Submit Note</button>
            </div>
        </div>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
            <div class="lg:col-span-1 flex flex-col space-y-4">
                <div>
                    <h3 class="text-lg font-bold text-gray-800 dark:text-gray-200 mb-2">1. Select Delivery Notes</h3>
                    
                    <form action="{{ route('receive-notes.create') }}" method="GET" class="mb-2 p-2 bg-gray-50 dark:bg-gray-700 rounded-md flex items-center space-x-2 text-sm">
                        ...
                        <div class="space-y-2 max-h-48 overflow-y-auto border border-gray-300 dark:border-gray-700 p-2 rounded-md">
                            @forelse($deliveryNotes as $dn)
                                <label class="flex items-center p-2 rounded-md hover:bg-gray-100 dark:hover:bg-gray-700">
                                    <input type="checkbox" name="delivery_note_ids[]" value="{{ $dn->id }}" x-model="selectedDeliveryNoteIds" @change="fetchItems" class="dark:bg-gray-900 rounded">
                                    <span class="ml-3 text-sm text-gray-700 dark:text-gray-300">{{ $dn->delivery_note_id }}</span>
                                </label>
                            @empty
                                <p class="text-sm text-gray-500 dark:text-gray-400 p-2">No delivered notes are pending receipt for the selected date range.</p>
                            @endforelse
                        </div>
                        <div class="mt-4 space-y-4">
                            <div>
                                <label for="received_date" class="block text-sm font-medium text-gray-700 dark:text-gray-300">Received Date*</label>
                                <input type="date" name="received_date" id="received_date" value="{{ old('received_date', date('Y-m-d')) }}" class="mt-1 block w-full dark:bg-gray-900 rounded-md py-2 px-3 border border-gray-300 dark:border-gray-600" required>
                            </div>
                             <div>
                                <label for="notes" class="block text-sm font-medium text-gray-700 dark:text-gray-300">Notes / Remarks</label>
                                <textarea name="notes" id="notes" rows="3" class="mt-1 block w-full dark:bg-gray-900 rounded-md py-2 px-3 border border-gray-300 dark:border-gray-600">{{ old('notes') }}</textarea>
                            </div>
                        </div>
                    </form>
                </div>
                            ...
                            <template x-for="(item, index) in items" :key="item.product_id">
                                <tr>
                                    ...
                                    <td class="px-4 py-2 text-sm text-right">
                                        <input type="number" :name="`items[${index}][quantity_received]`" x-model.number="item.quantity_received" class="w-24 dark:bg-gray-900 rounded-md py-1 px-2 border border-gray-300 dark:border-gray-600" form="receive-note-form">
                                    ...
                                    <td class="px-4 py-2 text-sm">
                                        <template x-if="item.quantity_received != item.quantity_expected">
                                            <input type="text" :name="`items[${index}][discrepancy_reason]`" placeholder="e.g., damaged" class="block w-full dark:bg-gray-900 rounded-md py-1 px-2 text-sm border border-gray-300 dark:border-gray-600" form="receive-note-form">
                                        </template>
                                    </td>
                                </tr>
                            </template>
                            <tr x-show="items.length === 0">
                                <td colspan="4" class="text-center py-4 text-sm text-gray-500">Select one or more delivery notes to see items.</td>
                            </tr>
                        </tbody>
                    </table>
                </div>
