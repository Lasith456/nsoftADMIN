...
@section('content')
<div class="container mx-auto p-2" x-data="bulkAgentPaymentForm()">
    <div class="bg-white dark:bg-gray-800 shadow-md rounded-lg p-4">
        <form id="bulk-agent-payment-form" action="{{ route('payments.storeBulkAgent') }}" method="POST">
            @csrf

            {{-- Header --}}
            <div class="flex justify-between items-center mb-4 pb-3 border-b dark:border-gray-700">
                <h2 class="text-2xl font-bold text-gray-800 dark:text-gray-200">Record Bulk Agent Payment</h2>
            ...
            {{-- Agent Selection & Payment Details --}}
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-4">
                <div>
                    <h3 class="text-lg font-bold text-gray-800 dark:text-gray-200 mb-2">1. Select Agent</h3>
                    <select name="agent_id" x-model="selectedAgentId" @change="fetchInvoices" class="mt-1 block w-full dark:bg-gray-900 rounded-md border-gray-300 dark:border-gray-600" required>
                ...
                <div class="grid grid-cols-2 gap-4">
                    <div>
                        ...
                        <input type="date" name="payment_date" value="{{ date('Y-m-d') }}" class="mt-1 block w-full dark:bg-gray-900 rounded-md border-gray-300 dark:border-gray-600" required>
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-gray-700 dark:text-gray-300">Payment Method</label>
                        <select name="payment_method" x-model="paymentMethod" class="mt-1 block w-full dark:bg-gray-900 rounded-md border-gray-300 dark:border-gray-600" required>
            ...
            {{-- Invoice Selection --}}
            <div x-show="selectedAgentId" x-cloak class="border-t pt-4">
                ...
                <div class="mt-4 max-h-80 overflow-y-auto border rounded-md">
                     <table class="w-full min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                        ...
                        <tbody>
                            ...
                            <template x-if="!isLoading && invoices.length === 0"><tr><td colspan="4" class="text-center p-4">No outstanding invoices found for this agent.</td></tr></template>
...
document.addEventListener('alpine:init', () => {
    Alpine.data('bulkAgentPaymentForm', () => ({
        ...
        init() {
            if (this.selectedAgentId) this.fetchInvoices();
        },
        fetchInvoices() {
            this.invoices = [];
            ...
            this.isLoading = true;
            fetch(`/agents/${this.selectedAgentId}/unpaid-invoices`)
                .then(res => res.json())
                .then(data => {
                    this.invoices = data;
        ...
        isFormValid() {
            return this.selectedAgentId && this.selectedInvoiceIds.length > 0 && this.amountPaid > 0;
        },
