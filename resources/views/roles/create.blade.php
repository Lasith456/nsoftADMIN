    ...
    <form method="POST" action="{{ route('roles.store') }}">
        ...
        <div class="space-y-6">
            <div>
                ...
                <input type="text" name="name" id="name" placeholder="Role Name" class="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500" value="{{ old('name') }}">
            </div>
                ...
                <div class="mt-2 grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4">
                    @foreach($permission as $value)
                        <label class="flex items-center space-x-3">
                            <input type="checkbox" name="permission[]" value="{{ $value->id }}" class="h-4 w-4 text-blue-600 border-gray-300 rounded focus:ring-blue-500">
